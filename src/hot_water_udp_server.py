...
Client sends a message to the rpi on my local net asking for the hot water
...
Host id 'localhost' or '127.0.0.1' connects with other processes on the same
computer.  The computer can have more then one network card (NIC) and every
NIC has own IP address. Use a single IP to receive requests/connection from
one NIC. Use Empty string or '0.0.0.0' to  receive requests from all NICs.
...
LOGFILE = "/tmp/hot_water_server.log"
...
def uwl_worker(uwl):
    ...
    """
    while True:
        data = cyl.read_sensors()
        uwl[0] = data['uwl']
        LOGGER.debug('uwl=%s', data)
        time.sleep(120)
